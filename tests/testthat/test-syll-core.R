...
test_that("caché: cuando está habilitado guarda y reutiliza resultados", {
  ...
  skip_if_not(exists("syll_set_options"))
  ...
  withr::defer(syll_set_options(cache_enabled = old_opt), teardown_env())

  syll_set_options(cache_enabled = TRUE)
  readabilityes:::.cache_clear()
  withr::defer(readabilityes:::.cache_clear(), teardown_env())

  w <- "cancion"
  cache_key <- paste0(syll_get_option("lang"), ":", w)
  expect_null(readabilityes:::.cache_get(cache_key))

  # Primera llamada: debe poblar el caché
  s1 <- syll_split(w)
  in_cache <- readabilityes:::.cache_get(cache_key)
