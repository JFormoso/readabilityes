...
test_that("cuenta tokens básicos con un silabificador trivial", {
  ...
  expect_equal(count_syllables(c("hola mundo", "qué tal"), syllable_fun = syll_one),
               c(2L, 2L))
})

test_that("maneja puntuación y espacios con drop_empty", {
  expect_equal(count_syllables("Hola,   mundo.", syllable_fun = syll_one), 2L)
  expect_equal(count_syllables("", syllable_fun = syll_one), 0L)
  expect_equal(count_syllables("   ", syllable_fun = syll_one), 0L)
})

test_that("manejo de NA según na_as_zero", {
  x <- c("uno dos", NA_character_, "tres")
  expect_equal(count_syllables(x, syllable_fun = syll_one), c(2L, NA_integer_, 1L))
  expect_equal(count_syllables(x, syllable_fun = syll_one, na_as_zero = TRUE), c(2L, 0L, 1L))
})

test_that("syll_args se propaga a syllable_fun", {
  # Con syll_len y weight=2, "ab cd" tiene 2+2 letras => (4)*2 = 8
  expect_equal(count_syllables("ab cd", syllable_fun = syll_len, syll_args = list(weight = 2)), 8L)
  # Sin weight (1 por defecto): 4
  expect_equal(count_syllables("ab cd", syllable_fun = syll_len), 4L)
})

test_that("vectoriza sobre múltiples entradas", {
  v <- c("uno dos", "tres", "cuatro cinco seis")
  expect_equal(count_syllables(v, syllable_fun = syll_one), c(2L, 1L, 3L))
})
